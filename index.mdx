...
<CardGroup cols={3}>
<Card title="Get API Key" icon="key" href="https://app.countrystatecity.in?source=sdk_docs&campaign=sdk_api_migration">
...
Use our offline npm and Python packages for bundled data with no API key or network requests, or the TypeScript SDK to call the live API in code for always-current data and search.

<CardGroup cols={2}>
<Card title="npm Packages" icon="npm" href="/api/sdks/npm">
  Offline data packages plus an API-backed CLI and TypeScript SDK
</Card>
...
<Card title="TypeScript SDK" icon="js" href="/api/sdks/typescript">
  Live API client for Node.js and browser — search, usage tracking, and always-current data
</Card>
</CardGroup>
...
<CardGroup cols={3}>
<Card title="Python API SDK" icon="python" href="/api/sdks/python#live-api-sdk">
  Build against live managed data with typed sync and async clients
</Card>
<Card title="All SDKs" icon="code" href="/api/sdks">
  JavaScript/TypeScript and Python packages shipped today, more languages planned
</Card>
...
<Step title="Get API Access">
  [Register for a free API key](https://app.countrystatecity.in?source=sdk_docs&campaign=sdk_api_migration) to start with 100 requests per day, or explore database installation guides for self-hosted solutions.
</Step>
