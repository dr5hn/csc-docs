...
erDiagram
    ...
    states ||--o{ postcodes : links
    cities ||--o{ postcodes : links
```
</div>
...
<ParamField path="code" type="varchar(20)" required>
The postal / ZIP code string, as stored (e.g., `90210`, `SW1A 1AA`, `110001`)
</ParamField>
...
<ParamField path="type" type="varchar(32)">
A source-provided classification, not a fixed database enum — new values can appear as more sources are imported. The current dataset uses `full` (complete postcode, most countries), `area` (broader geographic area, e.g. Netherlands, Iran, UK), `fsa` (Canadian Forward Sortation Area), `po_box`, and `street` (South Africa's distinct PO Box vs. street-delivery codes). See [Postcode Types](/database/postcodes#postcode-types) for the full breakdown.
</ParamField>
...
<ParamField path="city_id" type="integer">
...
<ParamField path="source" type="varchar(64)">
...
[
  {
    ...
    "state_id": 1416,
    "city_id": 111968,
    "latitude": 34.09030000,
    "longitude": -118.41250000,
    "source": "GeoNames",
    "wikiDataId": null
  },
  {
...
<Note>
  The postcodes table is available in the self-hosted database exports and via the hosted API — see [Get Postcode by Code](/api/endpoints/get-postcode-by-code) and [Get Postcodes by Country](/api/endpoints/get-postcodes-by-country). See the [postcodes reference](/database/postcodes) for coverage details and self-hosted query examples.
</Note>

<Info>
  **API access levels.** The hosted API tiers postcode fields the same way it tiers cities and countries: `id`, `code`, `country_code`, `state_code`, `locality_name`, and `type` are on every plan (**Basic**). `country_id`, `state_id`, `city_id`, `latitude`, and `longitude` require **Supporter+**. `source` and `wikiDataId` require **Professional+**. `created_at`, `updated_at`, and the internal normalized code used for matching are never exposed publicly. Self-hosted database exports are not access-tiered — all columns are present.
</Info>
