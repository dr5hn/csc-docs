...
<Tab title="Import & Query">
**Import using mongoimport:**

```bash
# Import countries
mongoimport --uri "mongodb://localhost:27017/geoDb" \
...
```javascript Node.js with MongoDB driver
...
const states = await db.collection('states')
  .find({ country_code: 'IN' })
  .sort({ name: 1 })
  .toArray();

// Get cities in Maharashtra
const cities = await db.collection('cities')
  .find({ country_code: 'IN', state_code: 'MH' })
  .project({ name: 1, latitude: 1, longitude: 1 })
  .toArray();

// Countries by region
const asianCountries = await db.collection('countries')
  .find({ region: 'Asia' })
  .sort({ name: 1 })
  .toArray();
```
  ...
  .then(data => {
    L.geoJSON(data, {
      pointToLayer: (feature, latlng) =>
        L.circleMarker(latlng, { radius: 5 }),
...
-- ogr2ogr -f PostgreSQL PG:"dbname=geoDb" cities.geojson

-- Or use ST_GeomFromGeoJSON
INSERT INTO city_points (name, country_code, geom)
SELECT
  f->>'name',
  f->>'country_code',
  ST_SetSRID(ST_GeomFromGeoJSON(f->'geometry'), 4326)
FROM jsonb_array_elements(:'geojson'::jsonb->'features') AS f;
```
</Tab>
</Tabs>
