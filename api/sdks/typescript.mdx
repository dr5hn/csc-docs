...
Requires **Node.js 18+** (for native `fetch`) or any modern browser. Zero runtime dependencies — built entirely on native `fetch`, `URL`/`URLSearchParams`, and `AbortController`. You'll need an API key — get one at [countrystatecity.in](https://countrystatecity.in) and compare [plans and quotas](https://countrystatecity.in/pricing).
