...
`dataVersion` matches the [Get Data Version](/api/endpoints/get-data-version) API endpoint's `dataVersion` field exactly, so you can confirm your bundled data and the live API are on the same release. The package's `recordCounts` is narrower than the API's, though — `countries`/`states`/`cities` only, no `regions`/`subregions`.
