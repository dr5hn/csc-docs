...
<Accordion title="What happens when I exceed my rate limit?">
  ...
  **Best practices for handling rate limits:**
  ...
  - Monitor usage proactively via the `X-CSC-Daily-Used`/`X-CSC-Daily-Limit` response headers
  ...
  <Tip>
  See [Errors & Rate Limits](/api/errors) for the full envelope shape, the monthly-limit variant, and usage limits per tier.
  </Tip>
</Accordion>
...
<Accordion title="What does a 403 Forbidden response mean?">
  ...
  <Note>
  `requiredTier` is occasionally `null` — that means your current tier normally includes this feature, but an account-specific override is denying it. Upgrading won't fix that; [contact support](mailto:api@countrystatecity.in) instead.
  </Note>
