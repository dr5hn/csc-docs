...
<Accordion title="What does a 403 Forbidden response mean?">
  ...
  <Note>
  `requiredTier` can be `null`. That means the API couldn't read the live plan catalogue while building the response, so it has no upgrade advice to give — the gate is still real. Use `details.upgradeUrl` (or the [pricing page](https://app.countrystatecity.in/pricing)) to find the plan that includes the feature.
  </Note>
