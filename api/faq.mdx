...
<Accordion title="Do you provide SDKs for different programming languages?">
  Yes. We maintain official live API clients for:

  - [TypeScript and JavaScript](/api/sdks/typescript)
  - [Python](/api/sdks/python#live-api-sdk)

  The REST API also works with any language that can make HTTP requests. We provide comprehensive [integration examples](/api/examples) for:
...
<Accordion title="What happens when I exceed my rate limit?">
  Exceeding your daily or monthly usage limit returns a `429 Too Many Requests` response:
  ...
  `details.resetAt` is an ISO 8601 UTC timestamp — next UTC midnight for a daily limit, the 1st of next UTC month for a monthly limit. Use it to schedule your retry instead of guessing a backoff window.

  **Best practices for handling rate limits:**
  - Read `details.resetAt` and wait until then before retrying
  - Cache responses to reduce API calls
  - Monitor usage proactively via the `X-CSC-Daily-Used`/`X-CSC-Daily-Limit` response headers on successful requests — they aren't returned on the `429` itself
  ...
  <Tip>
  See [Errors & Rate Limits](/api/errors) for the monthly-limit variant, the usage headers, and limits per tier.
  </Tip>
</Accordion>

<Accordion title="What does a 403 Forbidden response mean?">
  A `403` means your API key authenticated fine, but the request itself isn't allowed — most commonly because the endpoint, parameter, or feature you're using isn't included in your plan:
  ...
  Check `details.requiredTier` for the minimum plan that unlocks it, and `details.upgradeUrl` to upgrade directly.

  <Note>
  `requiredTier` can be `null`. That means there is no higher active plan the API can safely recommend, or the live plan catalogue could not be read. The gate is still real. Use `details.upgradeUrl` (or the [pricing page](https://app.countrystatecity.in/pricing)) to compare the available plans.
  </Note>

  <Warning>
  Not every `403` is a plan gate. A `403` raised for another reason returns `status` and `message` with no `details` object, so check `details` exists before reading `details.feature` or `details.requiredTier`.
  </Warning>

  <Tip>
  See [Errors & Rate Limits](/api/errors) for the full 403/429 reference.
  </Tip>
</Accordion>
