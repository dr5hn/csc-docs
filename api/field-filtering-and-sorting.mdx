...
<Note>
  `?fields=` and `?sort=` errors use the `status`/`message`/`details` envelope shown below. Other failures — the geographical `404`s in particular — return a bare `{"error": "..."}` object instead. See [Errors & Rate Limits](/api/errors) for both shapes and how to read either one.
</Note>

```json 400 - Unknown field in ?fields=
{
  "status": "error",
  "message": "Unknown field(s): fakeColumn. Valid fields: id, name, iso2, iso3, ..."
}
```
...
{
  "status": "error",
  "message": "Field \"timezones\" is not sortable. Sortable fields for countries: id, name, iso2, iso3, population, gdp, area_sq_km, latitude, longitude"
}
```
...
{
  "status": "error",
  "message": "Field \"population\" is not available on your current plan."
}
```
...
{
  "status": "error",
  "message": "Invalid sort token \"name:asc:extra\". Expected \"field\" or \"field:asc|desc\"."
}
```
