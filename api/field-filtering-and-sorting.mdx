...
<Note>
  See [Errors & Rate Limits](/api/errors) for the full `status`/`message`/`details` envelope every error response uses. The examples below use the messages specific to `?fields=`/`?sort=`.
</Note>
