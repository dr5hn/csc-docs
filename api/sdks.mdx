...
The Country State City ecosystem provides official packages for JavaScript/TypeScript and Python, plus a live API client SDK. The npm and Python packages embed the geographical data directly, so you can query countries, states, and cities without making HTTP requests or managing an API key. The TypeScript SDK is the opposite — a live REST API client for when you need always-current data, quota-aware usage, or capabilities like search that the offline packages don't have.
...
<CardGroup cols={2}>
  <Card title="Python" icon="python" color="#3776ab">
    Pythonic live API client library with async support
  </Card>
