...
<Tip>
**Trim and order results:** Add `?fields=` to limit columns returned, or `?sort=` to order the list. Both are available on **Starter+** plans. See the [Field Filtering & Sorting](/api/field-filtering-and-sorting) guide for syntax and per-entity sortable fields.
</Tip>
...
<ParamField query="q" type="string">
  Search filter on name. Case-insensitive match on `name` and `native` fields. Minimum 2 characters. **Requires Starter+ plan.** Without this parameter, all results are returned (no plan restriction for search).
</ParamField>

<ParamField query="kind" type="string">
  Filter by derived place classification. Comma-separated list of `settlement`, `administrative`, `section`, `unknown` (e.g. `?kind=settlement` or `?kind=settlement,section`). Free on every plan — no tier restriction. Invalid values return a `400` error listing the accepted values. See [City Types](/database/city-types) for what each value covers and how it's derived from `type`.
</ParamField>

<ParamField query="type" type="string">
  Filter by the raw source `type` value. Comma-separated list of one or more values from the [City Types](/database/city-types) reference (e.g. `?type=city,adm2`). **Requires Supporter+ plan** — matches the same tier gate as the `type` response field (full data access). On a lower plan the request is rejected with a `400` error rather than being ignored. Unrecognized values simply match zero rows rather than erroring.
</ParamField>
...
<ResponseField name="kind" type="string">
  Derived classification of the place: one of `settlement`, `administrative`, `section`, or `unknown`. Computed from `type` — available on every tier, including **Basic**. See [City Types](/database/city-types) for the full mapping.
</ResponseField>

<Info>City responses on the **Basic** tier return `id`, `name`, and `kind`. Upgrading to **Supporter+** unlocks the full field set: `state_id`, `state_code`, `country_id`, `country_code`, `latitude`, `longitude`, `timezone`, `population`, `type`, `level`, `parent_id`, `native`, `translations`, and `wikiDataId`. See **Tier-Based Field Availability** below.</Info>
...
```bash cURL (filter by kind)
curl -X GET 'https://api.countrystatecity.in/v1/countries/IN/states/MH/cities?kind=settlement' \
  -H 'X-CSCAPI-KEY: YOUR_API_KEY'
```
...
```python Python (filter by kind)
import requests

def get_settlements_by_state(country_code, state_code):
    response = requests.get(
      f'https://api.countrystatecity.in/v1/countries/{country_code}/states/{state_code}/cities',
    ...
    if response.ok:
        cities = response.json()
        print(f'Found {len(cities)} settlements in {state_code}, {country_code}')
        return cities
    else:
        print('State not found or no cities available')
        return []

cities = get_settlements_by_state('IN', 'MH')
```
...
```javascript JavaScript (filter by kind)
const getSettlementsByState = async (countryCode, stateCode) => {
  const response = await fetch(
    `https://api.countrystatecity.in/v1/countries/${countryCode}/states/${stateCode}/cities?kind=settlement`,
    { headers: { 'X-CSCAPI-KEY': 'YOUR_API_KEY' } }
  );

  if (response.ok) {
    const cities = await response.json();
    console.log(`Found ${cities.length} settlements in ${stateCode}, ${countryCode}`);
    return cities;
  } else {
    console.error('State not found or no cities available');
    return [];
...
[
  { "id": 133024, "name": "Mumbai", "kind": "settlement" },
  { "id": 133504, "name": "Pune", "kind": "settlement" },
  { "id": 133177, "name": "Nashik", "kind": "settlement" }
]
```

```json 200 - Full tier (Supporter, Professional, Business)
[
  {
    "id": 133024,
    "name": "Mumbai",
...
| **Full** | Supporter, Professional, Business | All Basic **+** `state_id`, `state_code`, `country_id`, `country_code`, `latitude`, `longitude`, `timezone`, `population`, `type`, `level`, `parent_id`, `native`, `translations`, `wikiDataId` |
