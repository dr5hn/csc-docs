...
<ParamField query="type" type="string">
  Filter by the raw source `type` value. Comma-separated list of one or more values from the [City Types](/database/city-types) reference (e.g. `?type=city,adm2`). **Requires Supporter+ plan** — matches the same tier gate as the `type` response field. Unrecognized values simply match zero rows rather than erroring.
</ParamField>
...
<Info>City responses on the **Basic** tier return `id`, `name`, and `kind`. Upgrading to **Supporter+** unlocks `state_id`, `state_code`, `country_id`, `country_code`, `latitude`, `longitude`, `timezone`, `population`, `type`, `level`, `parent_id`, and `native`. **Professional+** adds `translations` and `wikiDataId`. See **Tier-Based Field Availability** below.</Info>
...
[
  { "id": 133024, "name": "Mumbai", "kind": "unknown" },
  { "id": 132332, "name": "Pune", "kind": "settlement" },
  { "id": 133351, "name": "Nashik", "kind": "settlement" }
]
```
