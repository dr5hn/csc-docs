...
<Note>**Availability:** Professional and Business plans. Returns `403` on Community, Starter, Supporter, and Legacy plans. See **Trying it without a paid plan** below.</Note>

<Info>Matching uses the same PostgreSQL trigram search as [Fuzzy Search](/api/endpoints/fuzzy-search), against both the English `name` and the `native` name. Responses are cached server-side for up to an hour, keyed by query, type, country/state filter, limit, and your plan's data-access level — so a lower-tier response can never leak into a higher-tier cache hit or vice versa.</Info>
...
<ParamField query="state" type="string">
  Restrict city results to a single state. Requires `country` to also be set — sending `state` without `country` returns `400`, since state codes aren't globally unique across countries.
</ParamField>
...
<Note>
  `kind` (settlement/administrative/section/unknown filtering — see [City Types](/database/city-types)) and a `language` param for localized labels are both planned but not available on this endpoint yet.
</Note>
...
<ResponseField name="matched_field" type="string">
  Which name field actually matched: `name` or `native`.
</ResponseField>
...
A separate **Playground** plan powers the interactive API documentation — a basic-tier field set on a shared rate limit sized for many concurrent visitors, not a per-visitor quota. It isn't something you can sign up for directly; it's provisioned for the docs playground itself. For your own integration, autocomplete requires a Professional or Business plan.
