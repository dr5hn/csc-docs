...
<Tip>
  **Trim results:** Add `?fields=` to limit the columns returned. Requires **Starter+**. There is no `?sort=` on this endpoint — results are always ordered by code, then `id`, so cursor pagination stays stable. See the [Field Filtering & Sorting](/api/field-filtering-and-sorting) guide for `?fields=` syntax.
</Tip>
...
<ParamField query="q" type="string">
  Case-insensitive search: matches postcodes whose code starts with `q`, or whose locality name contains it. 2-100 characters. Optional — omit it to list the country's postcodes instead of searching. Every request to this endpoint requires Starter+ regardless of whether `q` is present.
</ParamField>
...
<ResponseField name="data[].source" type="string | null">
  Originating data source, for attribution and license tracking. **Requires Supporter+.**
</ResponseField>

<ResponseField name="data[].wikiDataId" type="string | null">
  Wikidata Q-ID for cross-referencing. **Requires Supporter+.**
</ResponseField>
...
```json 200 - Search with q, Basic tier (Starter)
{
  "data": [
    {
      "id": "131878",
...
```json 200 - Search with q, Full tier (Supporter, Professional, Business)
{
  "data": [
    {
      "id": "131878",
...
Community and Legacy lack the `searchEndpoint` feature this endpoint requires, so the **Basic** row below is only reachable on **Starter** — Community and Legacy get a `403` on this endpoint before field access matters.

| Tier | Plans | Fields |
|------|-------|--------|
| **Basic** | Starter | `id`, `code`, `country_code`, `state_code`, `locality_name`, `type` |
| **Full** | Supporter, Professional, Business | All Basic **+** `country_id`, `state_id`, `city_id`, `latitude`, `longitude`, `source`, `wikiDataId` |
...
This `403` fires on **every** request to this endpoint from a Community or Legacy key — including a bare list with no `q`, `state_code`, `city_id`, or `type` at all. Only exact lookup ([Get Postcode by Code](/api/endpoints/get-postcode-by-code)) is free on those plans.
...
<Note>
  This endpoint requires a Starter+ plan, for listing as well as searching. [Compare plans](https://countrystatecity.in/pricing?source=docs&campaign=postcode_search) to add it to your API key.
</Note>
