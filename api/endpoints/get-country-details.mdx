...
<Tip>
**Trim response columns:** Add `?fields=name,iso2,...` to receive only the columns you need. Available on **Starter+** plans. See the [Field Filtering & Sorting](/api/field-filtering-and-sorting) guide for syntax.
</Tip>
...
<ResponseField name="emojiU" type="string">
  Unicode representation of the flag emoji. **Full access level only.**
</ResponseField>
...
<ResponseField name="population" type="integer">
  Population estimate. **Full access level only.**
</ResponseField>

<ResponseField name="gdp" type="integer">
  Nominal GDP in USD. **Full access level only.**
</ResponseField>

<ResponseField name="area_sq_km" type="integer">
  Surface area in square kilometres. **Full access level only.**
</ResponseField>

<ResponseField name="postal_code_format" type="string">
  Display template for postal codes (`#` = digit, `@` = letter). **Full access level only.**
</ResponseField>

<ResponseField name="postal_code_regex" type="string">
  Regex for postal-code validation. **Full access level only.**
</ResponseField>
