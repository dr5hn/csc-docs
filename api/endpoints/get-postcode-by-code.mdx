...
<Note>
  **Free on every plan**, including Community. Exact lookup counts against your normal daily/monthly quota like any other request. Paginated listing and search use [Get Postcodes by Country](/api/endpoints/get-postcodes-by-country) and require a Starter+ plan — exact lookup has no such gate.
</Note>
...
<ResponseField name="data[].country_id" type="string">
  Country ID, as a numeric string. **Full access level only.**
</ResponseField>

<ResponseField name="data[].state_id" type="string | null">
  State ID, as a numeric string. **Full access level only.**
</ResponseField>

<ResponseField name="data[].city_id" type="string | null">
  City ID, as a numeric string, when the postcode is linked to a specific city. **Full access level only.**
</ResponseField>

<ResponseField name="data[].latitude" type="number | null">
  Latitude of the postcode area. **Full access level only.**
</ResponseField>

<ResponseField name="data[].longitude" type="number | null">
  Longitude of the postcode area. **Full access level only.**
</ResponseField>

<ResponseField name="data[].source" type="string | null">
  Originating data source, for attribution and license tracking. **Full access level only.**
</ResponseField>

<ResponseField name="data[].wikiDataId" type="string | null">
  Wikidata Q-ID for cross-referencing. **Full access level only.**
</ResponseField>
...
```json 200 - Duplicate exact match, Basic tier (Community, Starter, Legacy)
{
  "data": [
    {
      "id": "670115",
...
```json 200 - Duplicate exact match, Full tier (Supporter, Professional, Business)
{
  "data": [
    {
      "id": "670115",
...
<CardGroup cols={2}>
  <Card title="Get Postcodes by Country" icon="magnifying-glass" href="/api/endpoints/get-postcodes-by-country">
    Page through or search a country's postcodes. Requires Starter+.
  </Card>
